---
title: "`r TrainingDateString`: BDS Fallscheibe/`r weaponConfig$fallscheibe.type` -- 25m -- `r weaponConfig$weapon`"
...
kable(buildFallscheibeSummaryTable(pointGroups, pointDataConfig, weaponConfig, reportConfig, weaponConfig$fallscheibe.type,  NULL), format = "html", output = F, align = c("c", "c", "c", "c","c","c","c","c","c"), escape=F, table.attr="class='table table-striped table-hover'")
```

Bei Fallscheibe wird in diesem Report nur die Trefferlage analysiert. Für einen Treffer werden fiktiv 10 Ringe vergeben, für einen Fehlschuss 0 Punkte. Eine hoher Ringwert erlaub zukünftig eine schnellere Schussfrequenz.
...
kable(buildFallscheibeSummaryTable(pointGroups, pointDataConfig, weaponConfig, reportConfig,weaponConfig$fallscheibe.type,  correctionClicks), format = "html", output = F, align = c("c", "c", "c", "c","c","c","c","c","c"), escape=F, table.attr="class='table table-striped table-hover'")
```
...
- *Median* der typische Ring, also der am häufigsten getroffene Ring.
...
Die statistische Signifikanz, also die Aussagekräftigkeit, der zugrundeliegenden Berechnungen ist *`r calcHotellingSigni(pointGroups, pointDataConfig, weaponConfig, reportConfig)`*.
...
### Trefferdarstellung auf einer Fallscheibe-Darstellung
...
if (is.null(caliber) || caliber <= 0) {
  ...
  scheibe = NULL
  if(weaponConfig$fallscheibe.type == 'KK') {
    scheibe = fallscheibeKKTarget
  } else {
    scheibe = fallscheibeGKTarget
  }
...
simRing = simRingCount_Fallscheibe(getXYmat(pointGroups), target = scheibe, calSize = caliber, weaponConfig$fallscheibe.type)
...
Geschossen wurden *`r simuliert['10']`* Treffer und *`r simuliert['0']`* Fehlschüsse, bzw. nach Umrechnung auf 30 Wettbewerbsschüsse *`r round((simuliert['10']+0.00001)/shots*30)`* Treffer und *`r round((simuliert['0']+0.00001)/shots*30)`* Fehlschüsse.
